		...
		savData %>%
			imap(function(column, columnName) {
				...
				tibble_row(
					...
					min = (if (is.numeric(column)) {if (!all(is.na(column))) {min(column, na.rm = TRUE)} else {as(NA, typeof(column))}} else {NA_integer_}) %>% list(),
					max = (if (is.numeric(column)) {if (!all(is.na(column))) {max(column, na.rm = TRUE)} else {as(NA, typeof(column))}} else {NA_integer_}) %>% list(),
					sampleAnswers = list(sampleAnswers),
					choiceType,
					choiceCount = if (is.null(choices)) {NA_integer_} else {length(choices)},
					minChoice = (if (is.numeric(choiceLabels)) {if (!all(is.na(choiceLabels))) {min(choiceLabels, na.rm = TRUE)} else {as(NA, typeof(choiceLabels))}} else {NA_integer_}) %>% list(),
					maxChoice = (if (is.numeric(choiceLabels)) {if (!all(is.na(choiceLabels))) {max(choiceLabels, na.rm = TRUE)} else {as(NA, typeof(choiceLabels))}} else {NA_integer_}) %>% list(),
